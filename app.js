...
function presentCountires(countries) {
  // display countries in div//
 boxArea.innerHTML = "";
  countries.map(function (country) {
   ...
   boxArea.append(box);
    ...
    lan.innerHTML += country.languages;
    population.innerHTML += country.population.toLocaleString();
    flag.src += country.flag;
    //style//
    box.style.cssText =
      "border-radius: 30px; padding: 30px; opacity:75%; background-color:white; width:22%; display:flex; flex-direction:column; align-items:center; margin-bottom: 40px;  ";
  });
}
//display all countries
presentCountires(countriesObject);

//function filter by user input
let countryInput = document.querySelector("input");

countryInput.addEventListener("keyup", function (event) {
  let userInputLowerCase = countryInput.value.toLowerCase();
  filterByUserInput(userInputLowerCase);
})

//flter countries by user input
function filterByUserInput(userInput) {
  let filteredCountriesByUserInput = countriesObject.filter(function (country) {
    return country.name.toLowerCase().startsWith(userInput);
  })
  //display filtered countries
  presentCountires(filteredCountriesByUserInput);
  console.log(filteredCountriesByUserInput);
}


// sort by name//
btn[0].addEventListener('click',function(){
  let nameBySort = countriesObject.reverse(function(a,b){
    let countryA = a.name;
    let countryB = b.name;
  ...
  presentCountires(nameBySort);
  console.log(nameBySort);
})


//sort by capital//
function sortByCapital(capitalSort) {
  if(capitalAZ == false){
    capitalAZ = true;
    capitalSort=countriesObject.sort((a,b) => a.capital < b.capital? 1: -1)
  }else{
    capitalAZ = false
    capitalSort=countriesObject.sort((a,b) => a.capital > b.capital? 1: -1)
  }
  presentCountires(capitalSort);
}
...
btn[2].addEventListener('click', function(){
  if(popuAZ == false){
     popuAZ = true;
    countriesPop=  countriesObject.sort((a,b) => a.population > b.population ? 1: -1)
  }else {
    popuAZ = false;
    console.log(popuAZ);
    countriesPop = countriesObject.sort((a,b) => a.population < b.population ? 1: -1)
  }
  presentCountires(pop)
})
